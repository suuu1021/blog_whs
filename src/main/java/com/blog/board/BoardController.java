...
public class BoardController {
    ...
    // 게시글 상세보기
    @GetMapping("/board/{id}")
    public String detail(@PathVariable(name = "id")Long id, @RequestAttribute(value = Define.SESSION_USER, required = false) SessionUser sessionUser, Model model) {
    ...
    public String updateForm (@PathVariable(name = "id") Long id,
                              @RequestAttribute(value = Define.SESSION_USER) SessionUser sessionUser, Model model) {
        Board board = boardService.findByIdWithReplies(id, sessionUser);
        model.addAttribute("board", board);
    ...
    public String update(@PathVariable(name = "id")Long id, @Valid BoardRequest.UpdateDTO updateDTO, Errors errors,
                         @RequestAttribute(value = Define.SESSION_USER) SessionUser sessionUser) {
